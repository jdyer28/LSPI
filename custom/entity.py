...
class SmartSkid(metadata.BaseCustomEntityType):
    '''
    This is an experimental custom entity to model a smart skid
    '''
    ...
    def __init__(self, name, db, generate_days=0, db_schema=None, description=None, drop_existing=False):
        ...
        columns.append(Column('agitator', Float()))  # yes
        columns.append(Column('alarmresetbutton', Float()))  # yes
        columns.append(Column('ambienttemperature', Float()))  # yes
        # columns.append(Column('customerinputspeedrequest_train1', Float()))
        # columns.append(Column('customerinputspeedrequest_train2', Float()))
        # columns.append(Column('customerinputstartcommand_train1', Float()))
        # columns.append(Column('customerinputstartcommand_train2', Float()))
        # columns.append(Column('customerinputstopcommand_train1', Float()))
        # columns.append(Column('customerinputstopcommand_train2', Float()))
        # columns.append(Column('device_id', String(50)))
        columns.append(Column('feedpumppressure_train1', Float()))  # yes
        columns.append(Column('feedpumppressure_train2', Float()))  # yes
        # columns.append(Column('feedpumpswitch_train1', Float()))
        # columns.append(Column('feedpumpswitch_train2', Float()))
        columns.append(Column('flowmeter_train1', Float()))  # yes
        columns.append(Column('flowmeter_train2', Float()))  # yes
        columns.append(Column('injectionlevelswitch_train1', Float()))  # yes
        columns.append(Column('injectionlevelswitch_train2', Float()))  # yes
        columns.append(Column('injectionpumppressure_train1', Float()))  # yes
        columns.append(Column('injectionpumppressure_train2', Float()))  # yes
        # columns.append(Column('motorprotectiontripped_train1', Float()))
        # columns.append(Column('motorprotectiontripped_train2', Float()))
        # columns.append(Column('pressureswitch101', Float()))
        # columns.append(Column('pressureswitch102', Float()))
        columns.append(Column('processshutdownbutton', Float()))  # yes
        columns.append(Column('powerfailure', Float()))  # yes
        columns.append(Column('tanklevel', Float()))  # yes
        # columns.append(Column('tanklevel_ess',Float()))
        columns.append(Column('totalizerdaily_train2', Float()))  # yes
        columns.append(Column('totalizerlifetime_train2', Float()))  # yes
        columns.append(Column('vfd_train1', Float()))  # yes
        columns.append(Column('vfd_train2', Float()))  # yes
        # columns.append(Column('updated_utc', DateTime()))

        entity_ids = ['LP-1000', 'LP-1001', 'LP-1002', 'LP-1003', 'LP-1004']

        functions = []
        generator = bif.EntityDataGenerator(ids=entity_ids)
        ...
        dimension_columns = [
            ...
            # Column('numtanks', Float()),
            # Column('tanktype', Float())
        ]
...
###############################################
######### Simulated Smart Skid Entity #########
###############################################

class SimSmartSkid(metadata.BaseCustomEntityType):
    '''
    This is an experimental custom entity to model a smart skid
    '''

    def __init__(self, name, db, generate_days=10, db_schema=None, description=None, drop_existing=False):
        # Constants
        constants = []

        columns = []
        columns.append(Column('agitator', Float()))
        columns.append(Column('alarmresetbutton', Float()))
        columns.append(Column('ambienttemperature', Float()))
        columns.append(Column('customerinputspeedrequest_train1', Float()))
        columns.append(Column('customerinputspeedrequest_train2', Float()))
        columns.append(Column('customerinputstartcommand_train1', Float()))
        columns.append(Column('customerinputstartcommand_train2', Float()))
        ...
        columns.append(Column('customerinputstopcommand_train2', Float()))
        # columns.append(Column('device_id', String(50)))
        columns.append(Column('feedpumppressure_train1', Float()))
        columns.append(Column('feedpumppressure_train2', Float()))
        columns.append(Column('feedpumpswitch_train1', Float()))
        columns.append(Column('feedpumpswitch_train2', Float()))
        columns.append(Column('flowmeter_train1', Float()))
        columns.append(Column('flowmeter_train2', Float()))
        columns.append(Column('injectionlevelswitch_train1', Float()))
        columns.append(Column('injectionlevelswitch_train2', Float()))
        columns.append(Column('injectionpumppressure_train1', Float()))
        columns.append(Column('injectionpumppressure_train2', Float()))
        columns.append(Column('motorprotectiontripped_train1', Float()))
        columns.append(Column('motorprotectiontripped_train2', Float()))
        columns.append(Column('pressureswitch101', Float()))
        columns.append(Column('pressureswitch102', Float()))
        columns.append(Column('processshutdownbutton', Float()))
        columns.append(Column('tanklevel', Float()))
        columns.append(Column('tanklevel_ess', Float()))
        columns.append(Column('totalizerdaily_train2', Float()))
        columns.append(Column('totalizerlifetime_train2', Float()))
        columns.append(Column('vfd_train1', Float()))
        columns.append(Column('vfd_train2', Float()))
        ...
        sim = {
            'data_item_mean': {
                ...
                'customerinputspeedrequest_train1': 80,
                'customerinputspeedrequest_train2': 80,
                'feedpumppressure_train1': 20,
                'feedpumppressure_train2': 24,
                'flowmeter_train1': 4,
                'flowmeter_train2': 5,
                'injectionpumppressure_train1': 10,
                'injectionpumppressure_train2': 15,
                'tanklevel': 50,
                'tanklevel_ess': 40,
                'totalizerdaily_train2': 30,
                'totalizerlifetime_train2': 60,
                'vfd_train1': 8,
                'vfd_train2': 3,
            },
            'data_item_sd': {
                'ambienttemperature': 10,
                'customerinputspeedrequest_train1': 15,
                'customerinputspeedrequest_train2': 10,
                'feedpumppressure_train1': 5,
                'feedpumppressure_train2': 3,
                'flowmeter_train1': 1,
                'flowmeter_train2': 1.2,
                'injectionpumppressure_train1': 2,
                'injectionpumppressure_train2': 3,
                'tanklevel': 10,
                'tanklevel_ess': 80,
                'totalizerdaily_train2': 10,
                'totalizerlifetime_train2': 15,
                'vfd_train1': 1,
                'vfd_train2': 0.5
            },
            'data_item_domain': {
                'customer_id': ['123', '456', '789'],
                'pipeline': ['keystone', 'longhorn', 'bigpipe'],
                'numtanks': ['1', '2', '3'],
                'tanktype': ['1', '2', '3']
            }
        }
        ...
        functions = []
        generator = bif.EntityDataGenerator(ids=entity_ids, **sim)
        functions.append(generator)

        # Generating fake data for boolean features
        functions.append(bif.RandomDiscreteNumeric(
            discrete_values=[0, 1],
            probabilities=[0.9, 0.1],
            output_item='agitator'
        ))

        functions.append(bif.RandomDiscreteNumeric(
            discrete_values=[0, 1],
            probabilities=[0.99, 0.01],
            output_item='alarmresetbutton'
        ))

        functions.append(bif.RandomDiscreteNumeric(
            discrete_values=[0, 1],
            probabilities=[0.99, 0.01],
            output_item='customerinputstartcommand_train1'
        ))

        functions.append(bif.RandomDiscreteNumeric(
            discrete_values=[0, 1],
            probabilities=[0.99, 0.01],
            output_item='customerinputstartcommand_train2'
        ))

        functions.append(bif.RandomDiscreteNumeric(
            discrete_values=[0, 1],
            probabilities=[0.99, 0.01],
            output_item='customerinputstopcommand_train1'
        ))

        functions.append(bif.RandomDiscreteNumeric(
            discrete_values=[0, 1],
            probabilities=[0.99, 0.01],
            output_item='customerinputstopcommand_train2'
        ))

        functions.append(bif.RandomDiscreteNumeric(
            discrete_values=[0, 1],
            probabilities=[0.99, 0.01],
            output_item='feedpumpswitch_train1'
        ))

        functions.append(bif.RandomDiscreteNumeric(
            discrete_values=[0, 1],
            probabilities=[0.99, 0.01],
            output_item='feedpumpswitch_train2'
        ))

        functions.append(bif.RandomDiscreteNumeric(
            discrete_values=[0, 1],
            probabilities=[0.99, 0.01],
            output_item='injectionlevelswitch_train1'
        ))

        functions.append(bif.RandomDiscreteNumeric(
            discrete_values=[0, 1],
            probabilities=[0.99, 0.01],
            output_item='injectionlevelswitch_train2'
        ))

        functions.append(bif.RandomDiscreteNumeric(
            discrete_values=[0, 1],
            probabilities=[0.99, 0.01],
            output_item='motorprotectiontripped_train1'
        ))

        functions.append(bif.RandomDiscreteNumeric(
            discrete_values=[0, 1],
            probabilities=[0.99, 0.01],
            output_item='motorprotectiontripped_train2'
        ))

        functions.append(bif.RandomDiscreteNumeric(
            discrete_values=[0, 1],
            probabilities=[0.99, 0.01],
            output_item='pressureswitch101'
        ))

        functions.append(bif.RandomDiscreteNumeric(
            discrete_values=[0, 1],
            probabilities=[0.99, 0.01],
            output_item='pressureswitch102'
        ))

        functions.append(bif.RandomDiscreteNumeric(
            discrete_values=[0, 1],
            probabilities=[0.999, 0.001],
            output_item='processshutdownbutton'
        ))
        ...
        dimension_columns = [
            Column('customer_id', String(50)),
            Column('pipeline', String(50)),
            Column('numtanks', String(50)),
            Column('tanktype', String(50))
        ]
