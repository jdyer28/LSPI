...
class DRAUsageMonthly(BaseTransformer):
    """
    Calculation the DRA Usage for a skid per calendar month
    """
    def __init__(self, timestamp, totalizer_hourly, output_item):
        self.timestamp = timestamp
        self.totalizer_hourly = totalizer_hourly
        self.output_item = output_item
        super().__init__()

    def execute(self, df):
        dfc = df.copy()
        dfc['Month'] = dfc[self.timestamp].dt.month
        dfc[self.output_item] = dfc.groupby(['Month']).sum(self.totalizer_hourly)
        return dfc
...
# class DRAUsagePipeline(BaseTransformer):
#     """
#     Calculate the DRA Usage per pipeline per calendar day, week, and month
#     """
#
#     def __init__(self, pipeline, totalizer_hourly, output_item):
...
#     def execute(self, df):
#         dfc = df.copy()
#         dfc[self.output_item] = dfc[self.input_item_1] * dfc[self.input_item_2]
#         return dfc
#
#     @classmethod
#     def build_ui(cls):
